...
type MotionDivProps = { children: ReactNode; className?: string; delay?: number };
type FAQItemProps = {
...
const MotionDiv = ({ children, className, delay = 0 }: MotionDivProps) => {
  const { ref, inView } = useInView({
    triggerOnce: true,
    threshold: 0.1,
  });
    ...
    <main className="container min-h-screen mx-auto flex flex-col justify-start items-center">
      ...
      <Section className="flex flex-col gap-10 mt-40 px-4 md:px-0">
        <MotionDiv className="flex flex-col justify-center items-center gap-2">
          <motion.div variants={itemVariants} className="bg-white w-fit tracking-pill font-bold flex justify-center items-center px-4 py-1 rounded-full shadow-slighter"><div className="w-4 h-4 rounded-full [background:radial-gradient(35%_63%_at_50%_50%,rgb(99,102,241)_0%,rgb(67,56,202)_100%)] inline-block mr-2 animate-pulse duration-1000"></div><span className="text-sm md:text-[18px] text-fancy">Projekteket Várunk Májusra</span></motion.div>
          ...
          <motion.h2 variants={itemVariants} className="mt-4 mb-6 text-base md:text-[18px] [line-height:27px] font-semibold tracking-text text-gray max-w-xs text-center">
      ...
      <Section id="benefits" className="mt-20 flex flex-col justify-center items-center gap-10 font-bold text-4xl md:text-[56px] tracking-subtitle max-w-2xl text-center [line-height:1.1em] md:[line-height:60px] px-4 md:px-0">
        <MotionDiv>
          <h2 className="text-fancy mb-5">Amiért a Vállalkozások Minket Választanak</h2>
        </MotionDiv>
        <div className="flex flex-col lg:flex-row justify-between items-center gap-10">
          <MotionDiv className="relative group overflow-hidden w-full md:w-96 h-96 p-8 bg-white rounded-[42px] shadow-slighter flex flex-col justify-end items-start" delay={0.1}>
          ...
          <MotionDiv className="relative group w-full md:w-96 h-96 px-8 py-5 overflow-hidden bg-white rounded-[42px] shadow-slighter flex flex-col justify-start items-start" delay={0.2}>
          ...
          <MotionDiv className="w-full md:w-96 group overflow-hidden relative h-96 px-8 py-5 bg-white rounded-[42px] shadow-slighter flex flex-col justify-start items-start" delay={0.3}>
      ...
      <Section id="projects" className="mt-28 flex flex-col justify-center items-center gap-10 font-bold text-4xl md:text-[56px] tracking-subtitle w-full lg:w-5xl text-center [line-height:1.1em] md:[line-height:60px] px-4 md:px-0">
        <MotionDiv>
        ...
        <div className="flex flex-col justify-between items-center gap-10 w-full">
          <MotionDiv className="relative group overflow-hidden w-full gap-8 h-auto lg:h-96 p-8 bg-white rounded-[42px] shadow-slighter flex flex-col lg:flex-row justify-between items-start" delay={0.1}>
          ...
          <MotionDiv className="relative group overflow-hidden w-full gap-8 h-auto lg:h-96 p-8 bg-white rounded-[42px] shadow-slighter flex flex-col lg:flex-row justify-between items-start" delay={0.2}>
          ...
          <MotionDiv className="relative group overflow-hidden w-full gap-8 h-auto lg:h-96 p-8 bg-white rounded-[42px] shadow-slighter flex flex-col lg:flex-row justify-between items-start" delay={0.3}>
      ...
      <Section className="mt-28 flex flex-col justify-center items-center gap-10 font-bold text-4xl md:text-[56px] tracking-subtitle w-full md:w-5xl text-center [line-height:1.1em] md:[line-height:60px] px-4 md:px-0">
        <MotionDiv className="flex flex-col justify-center items-center gap-2">
          <motion.h1 variants={itemVariants} className="text-5xl md:text-7xl font-semibold tracking-[-0.06em] text-fancy-2 max-w-3xl text-center [line-height:1.1em] pb-3 pt-2">
            Átlátható Árazás Meglepetések Nélkül
          </motion.h1>
          <motion.h2 variants={itemVariants} id="pricing" className="mt-4 mb-6 text-base md:text-[18px] [line-height:27px] font-semibold tracking-text text-gray max-w-xl text-center">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-10">
          <MotionDiv className="relative overflow-hidden w-full md:w-96 h-auto px-8 py-5 bg-white rounded-[42px] shadow-slighter flex flex-col justify-start items-start" delay={0.1}>
            ...
            <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 }}}} className="flex flex-col justify-start items-start text-gray text-sm mt-8 gap-2 w-full">
              <motion.div variants={itemVariants} className="flex flex-row justify-start items-center gap-2">
              ...
              <motion.div variants={itemVariants} className="flex flex-row justify-start items-center gap-2 mb-4">
                <div className="select-none w-fit relative bg-gradient-to-b from-[#6366f1] to-[#3730a3] rounded-[47.86px] px-[3px] pt-[3px] pb-[3px]  shadow-slight group transition-all duration-300">
                  <div className="[background:radial-gradient(35%_63%_at_50%_50%,rgb(99,102,241)_0%,rgb(67,56,202)_100%)] rounded-[54.69px] tracking-text text-[18px] font-semibold text-white px-[4px] py-[4px] transition-all duration-300">
                    <Check strokeWidth={3} size={12}></Check>
                  </div>
          ...
          <MotionDiv className="relative w-full md:w-96 h-auto px-8 py-5 bg-white rounded-[42px] shadow-slighter flex flex-col justify-start items-start" delay={0.2}>
            <div className="absolute -bottom-40 left-0 w-full h-full">
          ...
          <MotionDiv className="relative w-full md:w-96 h-auto px-8 py-5 bg-white rounded-[42px] shadow-slighter flex flex-col justify-start items-start" delay={0.3}>
            <div className="absolute -bottom-40 left-0 w-full h-full">
      ...
      <Section id="faq" className="mt-28 flex flex-col lg:flex-row justify-center lg:justify-between items-center lg:items-start gap-10 font-bold text-4xl md:text-[56px] tracking-subtitle w-full lg:w-6xl text-center md:text-left [line-height:1.1em] md:[line-height:60px] px-4 md:px-0">
        <MotionDiv className="flex flex-col justify-center items-center lg:items-start lg:justify-start">
          <motion.h2 variants={itemVariants} className="font-semibold text-6xl md:text-[85px] text-fancy-3 [line-height:1.1em] md:[line-height:85px]">Kérdések?</motion.h2>
          <motion.h2 variants={itemVariants} className="font-semibold text-6xl md:text-[85px] text-fancy-3 [line-height:1.1em] md:[line-height:85px]">Válaszolunk.</motion.h2>
          <motion.p variants={itemVariants} className="text-gray text-lg md:text-xl text-center lg:text-left tracking-pill [line-height:25px] font-medium mt-5 max-w-md">Valami más érdekel? Küldj nekünk egyszerűen egy emailt a <a href="mailto:hello@reigindustries.com" className="text-[#312e81] font-semibold">hello@reigindustries.com</a> címre</motion.p>
        </MotionDiv>
      ...
      <Section className="mt-28 flex flex-row justify-between items-start gap-10 font-bold text-4xl lg:text-[56px] tracking-subtitle w-full lg:w-5xl text-center [line-height:1.1em] lg:[line-height:60px] px-4 lg:px-0">
        <MotionDiv className="bg-fancy w-full h-full rounded-t-4xl px-0 lg:px-8 pt-0 lg:pt-8" delay={0.2}>
          <div className="relative h-auto md:h-[40rem] overflow-hidden w-full bg-fancy-2 rounded-t-2xl pb-[9rem] pt-[2.25rem] px-4 md:px-[2.25rem] flex flex-col justify-start items-start">
            ...
            <motion.h1 variants={itemVariants} className="text-left w-full lg:w-3xl text-4xl md:text-6xl font-semibold text-fancy-4 [line-height:1.1em] md:[line-height:60px] pb-2">Alkossunk Valami Nagyszerűt Együtt Még Ma!</motion.h1>
            <motion.p variants={itemVariants} className="text-fancy-4 opacity-70 text-left text-lg md:text-xl mt-8 tracking-pill w-full md:w-xl font-semibold">Foglald le az ingyenes bevezető hívásodat, hogy elkezdhessük designolni, fejleszteni és optimalizálni a weboldaladat.</motion.p>
            <MotionDiv className="mt-8 flex flex-col md:flex-row justify-start items-start md:items-center gap-6 w-full" delay={0.3}>
              <WhiteCTAButton />
            ...
            <MotionDiv className="flex flex-col md:flex-row justify-between items-start md:items-center w-full mt-12 md:mt-24" delay={0.4}>
            <Image src={reigLogoLight} width={1000} height={1000} alt="Reig Industries" className="w-60 md:w-[20rem] mb-4 md:mb-0" />
            </MotionDiv>
            <MotionDiv className="flex flex-row justify-between items-center w-full mt-2" delay={0.5}>
